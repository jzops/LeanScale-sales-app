...
export default function ClayDiagnostic() {
  ...
  const processStats = countStatuses(clayProcesses);
  const priorityCount = clayProcesses.filter(p => p.addToEngagement).length;

  return (
    <Layout title="Clay Diagnostic Results">
      <div className="container">
        <div className="page-header" style={{ textAlign: 'center' }}>
          <h1 className="page-title" style={{ justifyContent: 'center' }}>
            <span>{'\uD83E\uDDF1'}</span> Clay Diagnostic Results
          </h1>
          <p className="page-subtitle">Clay enrichment and automation maturity assessment</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
          <div className="card" style={{ textAlign: 'center', padding: '1.25rem' }}>
            ...
            <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--ls-purple-light)' }}>{clayProcesses.length}</div>
          </div>
          <div className="card" style={{ textAlign: 'center', padding: '1.25rem' }}>
            <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: '0.25rem' }}>Categories</div>
            <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--ls-purple-light)' }}>{clayCategories.length}</div>
          </div>
          <div className="card" style={{ textAlign: 'center', padding: '1.25rem' }}>
        ...
        <section className="card" style={{ padding: '1.5rem', marginBottom: '2rem', background: 'linear-gradient(135deg, #1e1b4b 0%, #312e81 50%, #4c1d95 100%)' }}>
          ...
          <p style={{ color: '#c4b5fd', fontSize: '0.8rem', marginBottom: '1.25rem' }}>
            Status distribution across all Clay inspection points
          </p>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
            {[
              { label: 'Processes', stats: processStats, count: clayProcesses.length },
            ].map((item) => (
              <div
                key={item.label}
                style={{
        ...
        <div style={{ marginTop: '2rem' }}>
            ...
            <div>
              <div className="diagnostic-charts-row" style={{ marginBottom: '1.5rem' }}>
                ...
                <div className="card" style={{ padding: '1.5rem', overflow: 'auto' }}>
                  <BarChart data={clayProcesses} title="Process Health Overview" maxItems={31} />
                </div>
              </div>
              <div className="card" style={{ padding: '1.5rem' }}>
                <h3 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '1rem' }}>All Processes ({clayProcesses.length})</h3>
                <ItemTable items={clayProcesses} showFunction={true} />
              </div>
            </div>
          )}
          ...
          {activeTab === 'by-category' && (
            <div className="card" style={{ padding: '1.5rem' }}>
              ...
              <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--text-sm)', marginBottom: '1.5rem' }}>
                {clayProcesses.length} processes grouped by Clay category
              </p>
              <GroupedView items={clayProcesses} groupByField="function" groupNames={clayCategories} />
            </div>
          )}
          ...
          {activeTab === 'by-outcome' && (
            <div className="card" style={{ padding: '1.5rem' }}>
              ...
              <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--text-sm)', marginBottom: '1.5rem' }}>
                {clayProcesses.length} processes grouped by their desired business outcome
              </p>
              <GroupedView items={clayProcesses} groupByField="outcome" groupNames={clayOutcomes} />
            </div>
          )}
        </div>
        ...
        <div className="cta-banner" style={{ marginTop: '2rem' }}>
          <h3 className="cta-title">Ready to optimize your Clay implementation?</h3>
          ...
          <div className="cta-buttons">
            ...
            <Link href="/buy-leanscale/clay" className="btn cta-btn-secondary">
              Explore Clay x LeanScale
            </Link>
          </div>
        </div>
      </div>
