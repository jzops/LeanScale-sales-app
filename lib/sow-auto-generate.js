...
export function generateSectionsFromDiagnostic(processes, catalogMap = new Map()) {
  ...
  // Auto-generate timeline dates (sequential scheduling)
  assignTimelineDates(sections);
...
/**
 * Assign sequential timeline dates to sections based on estimated hours.
 * Assumes ~20 hours/week capacity. Sections without hours get 3 weeks default.
 */
function assignTimelineDates(sections) {
  ...
  for (const section of sections) {
    const durationWeeks = section.hours ? Math.ceil(section.hours / 20) : 3;

    section.startDate = cursor.toISOString().split('T')[0];

    const endDate = new Date(cursor);
    endDate.setDate(endDate.getDate() + durationWeeks * 7 - 1);
    section.endDate = endDate.toISOString().split('T')[0];
