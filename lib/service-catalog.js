...
/**
 * Get a single service by slug (e.g., "activity-capture")
 * Used to bridge static services-catalog.js IDs with DB records
 */
...
/**
 * Batch lookup services by slugs.
 * Returns a Map<slug, service> for efficient lookup during SOW generation.
 */
export async function getServicesBySlugs(slugs) {
  const map = new Map();
  if (!supabase || !slugs || slugs.length === 0) return map;
  ...
  try {
    ...
    for (const service of data) {
      if (service.slug) map.set(service.slug, service);
    }
    return map;
  } catch {
    return map;
